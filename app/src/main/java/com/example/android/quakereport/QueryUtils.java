...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquakes> extractEarthquakes() {
        ...
        try {
            JSONObject eq = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray features = eq.getJSONArray("features");
            for(int i=0;i<features.length();i++){
                JSONObject ceq = features.getJSONObject(i);
                JSONObject tmp = ceq.getJSONObject("properties");
                Double magnitude = tmp.getDouble("mag");
